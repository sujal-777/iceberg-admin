...
const Adding_questions = ({ data }: { data: TestSeriesItem }) => {
  ...
  const fetchquestionData = async () => {
    try {
      const res = await fetch(`http://localhost:5000/admin/questions/${data._id}`);
...
setOpenAddQuestion(false);
 ...
 const  handleAddQuestion=()=>{
  ...
  setOpenUpdateQuestion(true);
   ...
   const handleCloseUpdateQuestion=(data: boolean | ((prevState: boolean) => boolean))=>{
setOpenUpdateQuestion(false);
  ...
  return (
    <div className=''>
        ...
        <h1 className='text-[23px] font-[500] mb-[35px]'>{data.title} Mock MCQ Questions</h1>
        <table className='md:w-full overflow-x-scroll md:overflow-x-none'>
          <thead className=''>
            <tr>
            {['Questions','Marks','Actions'].map((item,index)=>(
              <th key={index} className='r border-[1px] border-b-black text-start border-t-black px-[10px] md:px-[36px]  text-[22px] font-[500]'>{item}</th>
            ))}
            </tr>
          </thead>
          ...
          (<tbody>
            {questionsData.map((item,index)=>(
             item.testSeriesId?.toString() === data._id?.toString() && (
      <tr key={index} className=' border-[1px] border-b-black '>
        <td className='  py-[16px] px-[10px] md:px-[36px]'>{index+1}.{item.questionText}</td>
        <td className='text-center md:pr-[90px]'>{item.marks}</td>
        ...
        <td className='flex  items-center gap-[20px] px-[10px] pt-[15px] md:px-[36px]'>
          <motion.button
                        ...
                        >
          <Edit className="w-4 h-4 text-blue-600" onClick={()=>handleUpdateQuestion(item._id)} /> 
          </motion.button>
             <motion.button
                          whileHover={{ scale: 1.1 }}
                          whileTap={{ scale: 0.9 }}
                        ...
                        >
          <Trash2 className="w-4 h-4 text-red-600" onClick={()=>deleteQuestion(item._id)}/>
          </motion.button>
          </td>
        <td></td>
      </tr>
          ...
          </tbody>):<tbody><tr className='mt-[10px]'><td>No Question added yet!</td></tr></tbody>}
        </table>
      </div>
      </div>}
        {openAddQuestion ===true &&<div className='absolute z-40 flex justify-center max-h-full inset-0 py-[25px] bg-black/30 backdrop-blur-sm w-full'><AddQuestion testseriesid={`${data._id}`} fromCancelButton={handleCloseAddQuestion} /></div>}
        {openUpdateQuestion ===true &&<div className='absolute z-40 flex justify-center max-h-full inset-0 py-[25px] bg-black/30 backdrop-blur-sm w-full'><Update_question questionid={updateQuestionId} fromCancelButton={handleCloseUpdateQuestion} /></div>}
       {questiontype==='Descriptive' && <div className='mt-[100px]'>
         <div className='flex flex-col md:flex-row justify-between'>
          <div className='flex flex-col md:flex-row '>
            ...
            <h1 className='text-[24px] font-[600]'>{data.title} Descriptive Questions</h1>
            <div>
